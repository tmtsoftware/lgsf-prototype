...
def start_server(xprocess):
    ...
    python_server_script_full_path = py.path.local(__file__).dirpath("../py-bto-prototype-hcd/BtoPrototypeHcdHandlers.py")
    ...
    xprocess.ensure("BtoPrototypeHcdHandlers", Starter)
    yield
    xprocess.getinfo("BtoPrototypeHcdHandlers").terminate()
...
class TestCommandServiceClientServer:
    ...
    async def test_command_client_server(self):
        ...
        cs = CommandService(Prefix(Subsystem.LGSF, "BtoPrototypeHcd"), ComponentType.Service, clientSession)
